...
- **Account deletion**: the profile screen's Delete Account flow (type `DELETE` to confirm) calls the `delete-account` edge function — JWT self-only, blocks on outstanding obligations (active tenancies, pending withdrawals/payments/applications), de-links the `transactions` ledger (amounts retained, payer/payee nulled for accounting), purges storage (avatar, owned listings' images), then deletes the auth user (FK cascades erase the rest). Extend the blocker list when new obligation tables arrive.
